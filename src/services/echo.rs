...
pub async fn echo(
    ...
    let prot = req.headers().get("x-forwarded-proto");
    let prot_str: &str = match prot {
    ...
    let path = "/echo";
    let url = format!("{}://{}{}", prot_str, hostname, path);

    match (req.method(), req.uri().path()) {
        // Serve some instructions at /
        (&Method::GET, "/") => Ok(Response::new(full(format!(
            "Try POSTing data to /echo such as: `curl {} -XPOST -d \"hello world\"`",
            url
        )))),
